...
public class PanelCashierController implements Initializable {
    ...
    private boolean isSessionFinished() {
        ...
        if (currentSession == null) {
            ...
            boolean isSessionOff = false;
            LocalDate today = LocalDate.now();

            if (currentSession.getId() != null) {
                if (currentSession.getDayInTime() != null) {
                    LocalDate sessionDate = currentSession.getDayInTime().toInstant()
                            .atZone(ZoneId.systemDefault())
                            .toLocalDate();
                    if (sessionDate.equals(today)) {
                        isSessionOff = "OFF".equals(currentSession.getStatus());
                    }
                }
            }
